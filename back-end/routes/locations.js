...
router.post('/', async (req, res) => {
  try {
    ...
    // Vérification liste noire
       if (clientDoc?.blacklisted && !req.body.overrideBlacklist) {
      return res.status(403).json({ 
        message: "Ce client est dans la liste noire. Veuillez contacter l'administrateur.",
        code: "CLIENT_BLACKLISTED",
        client: clientDoc // Envoyer les infos client pour affichage
      });
    }
    // Calcul de la durée
    ...
    const prixTTC = Number((prixAvecRemise * 1.19).toFixed(2)); 
...
const distanceParcourue = Number((kilometrageFinal - location.kilometrageInitial).toFixed(2));
...
location.kilometrageFinal = Number(kilometrageFinal.toFixed(2));
    ...
    // Notification
   await Notification.create({
  message: vidangeNecessaire
    ? `⚠️ Le véhicule ${location.vehiculeId.marque} ${location.vehiculeId.modele} a dépassé 10 000 km. Vidange nécessaire.`
    : `Location terminée: ${location.vehiculeId.marque} ${location.vehiculeId.modele}`,
  isAlert: vidangeNecessaire,
  category: vidangeNecessaire ? "maintenance" : "location"
});
...
// GET : Nombre de locations en cours
router.get('/stats/en-cours', async (req, res) => {
  try {
    const count = await Location.countDocuments({ statut: "active" });
    res.status(200).json({ nombreLocationsEnCours: count });
  } catch (error) {
    console.error("Erreur lors du comptage des locations en cours:", error);
    res.status(500).json({
      message: "Erreur serveur lors du comptage des locations en cours",
      error: error.message
    });
  }
});
