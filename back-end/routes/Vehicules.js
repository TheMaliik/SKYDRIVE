...
router.delete("/:id", async (req, res) => {
    ...
    try {
        ...
        // Supprimer les données associées dans Maintenance
        await Maintenance.deleteMany({ vehicule: id });

        // Supprimer les données associées dans Location
        await Location.deleteMany({ vehiculeId: id });
        ...
        res.status(200).json({ message: "Véhicule et données associées supprimés avec succès" });
    } catch (err) {
        res.status(500).json({ message: "Erreur lors de la suppression du véhicule", error: err.message });
    }
});
