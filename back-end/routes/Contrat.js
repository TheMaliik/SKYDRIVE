...
router.get('/total', async (req, res) => {
  try {
    ...
    res.json({ total: totalContracts });
  } catch (err) {
    res.status(500).json({ error: 'Erreur lors du comptage des contrats' });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (contract.publicId) {
      await cloudinary.uploader.destroy(contract.publicId, { resource_type: 'raw' });
    }

    // Delete contract from MongoDB
