...
// Route to count vehicles by age
router.get('/vehicles/count-by-age', async (req, res) => {
    try {
        const currentYear = new Date().getFullYear();
        
        const vehiclesByAge = await Vehicule.aggregate([
            {
                $project: {
                    age: { $subtract: [currentYear, '$annee'] } // Calculate age
                }
            },
            {
                $group: {
                    _id: '$age', // Group by calculated age
                    count: { $sum: 1 } // Count vehicles in each group
                }
            },
            {
        ...
        res.status(200).json({
            success: true,
            data: vehiclesByAge,
            message: 'Vehicles counted by age successfully'
        });
    } catch (error) {
        console.error('Error counting vehicles by age:', error);
        res.status(500).json({
            success: false,
            message: 'Error counting vehicles by age',
            error: error.message
        });
    }
});
...
// Route to count vehicles with status "Disponible"
router.get('/vehicles/available/count', async (req, res) => {
    try {
        const count = await Vehicule.countDocuments({ statut: 'Disponible' });
        res.status(200).json({ count });
    } catch (error) {
        console.error('Error counting available vehicles:', error);
        res.status(500).json({ message: 'Server error while counting available vehicles' });
    }
});


// Route to fetch all vehicles with availability details
router.get('/vehicles/availability', async (req, res) => {
  try {
    // Fetch all vehicles with relevant fields
    const vehicles = await Vehicule.find()
      .select('marque modele immatriculation statut carburant prixParJour')
      .lean();

    // Fetch active locations to determine availability
    const activeLocations = await Location.find({ statut: 'active' })
      .populate('clientId', 'nom prenom')
      .lean();

    // Process vehicles to include availability information
    const vehiclesWithAvailability = vehicles.map(vehicle => {
      const now = new Date();
      let isAvailable = vehicle.statut === 'Disponible';

      // Find active rentals for this vehicle
      const vehicleActiveLocations = activeLocations.filter(location => 
        location.vehiculeId.toString() === vehicle._id.toString()
      );

      // If there are active rentals, vehicle is not available
      if (vehicleActiveLocations.length > 0) {
        isAvailable = false;
      }

      // Format the response with booking details
      return {
        id: vehicle._id,
        ...
        isAvailable,
        bookings: vehicleActiveLocations.map(location => ({
          bookingId: location._id,
          client: location.clientId ? {
            id: location.clientId._id,
            nom: location.clientId.nom,
            prenom: location.clientId.prenom
          } : null,
          startDate: location.startDate,
          endDate: location.endDate,
          prixTTC: location.prixTTC,
