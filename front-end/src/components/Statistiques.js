...
const StatistiquesVehicules = () => {
  ...
  useEffect(() => {
    const fetchStatistiquesVehicules = async () => {
      try {
        ...
        const yearResponse = await axios.get('http://localhost:5000/api/stats/vehicles/count-by-age');
        if (!yearResponse.data || !Array.isArray(yearResponse.data.data)) {
          throw new Error('Réponse de données d\'âge invalide');
        }
        const yearData = yearResponse.data.data;
        const filteredYearData = yearData
          .filter(item => typeof item.age === 'number' && typeof item.count === 'number' && !isNaN(item.count) && item.count > 0);
        setYearChartData({
          series: filteredYearData.map(item => item.count),
          labels: filteredYearData.map(item => `${item.age} ans`),
          total: filteredYearData.reduce((sum, item) => sum + item.count, 0),
        });
        ...
        setKilometrageChartData({
          series: kilometrageResponse.data.map(item => parseFloat(item.avgKilometrage.toFixed(2))),
  ...
  const chartOptions = (title, labels, chartType = 'pie') => ({
    ...
    dataLabels: {
      enabled: true,
      formatter: chartType === 'pie' ? (val) => `${val.toFixed(2)}%` : (val) => val.toFixed(2),
      style: {
        fontSize: '12px',
        fontFamily: '"Inter", sans-serif',
        colors: chartType === 'pie' ? ['#ffffff'] : ['#263238'],
      },
      dropShadow: { enabled: chartType === 'pie', top: 1, left: 1, blur: 1, opacity: 0.5 },
    },
    responsive: [{
      breakpoint: 576,
      options: {
    ...
    ...(chartType === 'bar' ? {
      ...
      tooltip: {
        y: {
          formatter: (val) => `${val.toFixed(2)}${chartType === 'bar' && title.includes('Kilométrage') ? ' km' : ''}`
        }
      }
    } : {}),
  });
              ...
              {overallStats ? (
                <div className="stats-grid">
                  ...
                  <div className="stat-item">
                    ...
                    <span className="stat-value">{overallStats.averageRentalDurationDays.toFixed(2)} jours</span>
                  </div>
                  <div className="stat-item">
                    <span className="stat-label">Distance Moyenne</span>
                    <span className="stat-value">{overallStats.averageDistanceKm.toFixed(2)} km</span>
                  </div>
                </div>
              ) : (
        ...
        <div className="col">
          <div className="chart-card">
            ...
            <div className="card-body">
              {isYearValid ? (
                <Chart
                  options={chartOptions('Répartition par Âge', yearChartData.labels, 'bar')}
                  series={[{ name: 'Véhicules', data: yearChartData.series }]}
                  type="bar"
                  height={350}
                />
              ) : (
                <div className="no-data">Aucune donnée d'âge disponible</div>
              )}
            </div>
          </div>
        </div>
